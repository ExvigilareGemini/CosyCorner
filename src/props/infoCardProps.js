export const infoCardProps = [
  {
    index: "1",
    link: "/assets/img/nuitees_resultat.webp", 
    alt: "Nuitées",
    title: "Nuitées",
    text: "Les nuitées vous offrent un accès exclusif à l'ensemble de l'établissement et à toutes ses installations. Du jacuzzi au sauna infrarouge, de l'espace de luminothérapie au salon, en passant par la chambre avec lit rond, la table de massage, la cuisine et les deux salles de bain.",
  },
  {
    index: "2",
    link: "/assets/img/alheure_resultat.webp", 
    alt: "À l'heure",
    title: "À l'heure",
    text: "Pour les réservations à l'heure, l'accès comprend le sauna, le jacuzzi, le salon ainsi qu'une salle de bain équipée de douches. L'accès à la chambre avec lit rond, à la cuisine, à la table de massage et à la seconde sale de bain est possible en option, moyennant un supplément de 40€.",
  },
  {
    index: "3",
    link: "/assets/img/prestation_resultat.webp", 
    alt: "Prestations",
    title: "Prestations",
    text: "Des boissons telles que des cocktails individuels, bouteilles de champagne et une sélection de softs, ainsi que de petites gourmandises comme des chips ou des Raffaello sont à disposition tout au long du séjour. Ces options, payantes, peuvent être consommées librement selon les envies.",
  },
];
